...
const img=new Image(); let source=null, scale=1, doors=2, applied=false, finish=null;
...
function resetGeometry(){corners=DEFAULT_CORNERS.map(p=>({...p}));dividers=[];for(let i=1;i<doors;i++){const t=i/doors;dividers.push({top:{x:t,y:.14},bottom:{x:t,y:.86}})} }
...
function draw(){if(!source)return;ctx.clearRect(0,0,cv.width,cv.height);ctx.drawImage(source,0,0,cv.width,cv.height);if(applied&&finish)renderFinish();else {ctx.save();poly(...pts());ctx.strokeStyle='#35d28b';ctx.lineWidth=3;ctx.stroke();ctx.restore();drawGuides()}}
...
function renderFinish(){const p=pts();const img2=new Image();img2.onload=()=>{
  ...
  const tex=document.createElement('canvas');tex.width=tw;tex.height=th;const tc=tex.getContext('2d');tc.drawImage(img2,pad,pad,tw,th,0,0,tw,th);
  ctx.save();
  const pat=ctx.createPattern(tex,'repeat');
  const rot=+document.getElementById('rot').value;
  doorPolys().forEach((dp)=>{ctx.save();poly(...dp);ctx.clip();ctx.globalAlpha=+document.getElementById('opacity').value;ctx.fillStyle=pat;
    if(rot===90){ctx.translate(cv.width/2,cv.height/2);ctx.rotate(Math.PI/2);ctx.translate(-cv.width/2,-cv.height/2)}
    ctx.fillRect(-cv.width,-cv.height,cv.width*3,cv.height*3);ctx.restore();
  });
  ctx.restore();
  // subtle, realistic door seams; no bright white outline
  ctx.save();
  doorPolys().forEach(dp=>{poly(...dp);ctx.strokeStyle='rgba(0,0,0,.30)';ctx.lineWidth=1;ctx.stroke()});
  ctx.restore();
};img2.src='assets/'+finish+'.jpg'}
...
function setDoorCount(n){doors=n;while(dividers.length<doors-1){const i=dividers.length+1,t=i/doors;dividers.push({top:{x:t,y:.14},bottom:{x:t,y:.86}})}dividers=dividers.slice(0,doors-1);document.querySelectorAll('#doors button').forEach(x=>x.classList.toggle('on',+x.dataset.n===doors));applied=false;draw();renderHandles();setStatus(doors===1?'1 door selected — only the four corner handles are shown.':`${doors} doors selected — drag the colored divider handles to set each door separately.`)}
...
document.getElementById('apply').onclick=()=>{if(!source)return setStatus('Choose a photo first.');if(!finish)return setStatus('Select a finish first.');applied=true;draw();setStatus((names[finish]||finish)+' applied to '+doors+' separate door(s).')};
...
resetGeometry();renderCatalog();loadSrc('assets/sample.jpg','Sample loaded. Select a door count, then drag the divider handles.');
})();
